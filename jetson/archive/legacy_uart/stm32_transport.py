#!/usr/bin/env python3
"""Framed Jetson -> STM32 slot-command transport.

Command (Jetson -> STM32), 5 bytes::

    AA 55 command_id slot crc8(command_id, slot)

Response (STM32 -> Jetson), 5 bytes::

    AA 55 command_id status crc8(command_id, status)

Retries reuse the same command_id so the STM32 can suppress duplicate robot
motion while still returning the previous ACK/DONE result.
"""
...
def build_response(command_id: int, status: int) -> bytes:
    """Helper used by tests and the matching STM32 protocol documentation."""
    if not 0 <= command_id <= 0xFF:
        raise ValueError("command_id must be 0..255")
...
class Stm32Transport:
    def __init__(
        ...
        if baudrate <= 0 or ack_timeout <= 0 or done_timeout <= 0 or retries < 0:
            raise ValueError("invalid STM32 serial/timeout/retry setting")
        ...
        while time.monotonic() < deadline:
            while True:
                ...
                try:
                    ...
                    print(f"[STM32 DROP] {exc} frame={frame.hex(' ').upper()}")
    ...
    def send_slot_command(self, command_id: int, slot: int) -> CommandResult:
        ...
        for attempt in range(1, total_attempts + 1):
            ...
            print(
                f"[STM32 TX] attempt={attempt}/{total_attempts} "
                f"command_id={command_id} slot={slot} frame={frame.hex(' ').upper()}"
            )

            status = self._read_response(command_id, self.ack_timeout)
            if status is None:
                print(f"[STM32 TIMEOUT] ACK command_id={command_id}")
                continue
            print(f"[STM32 RX] command_id={command_id} status={STATUS_NAMES[status]}")
            ...
            if status is None:
                print(f"[STM32 TIMEOUT] DONE command_id={command_id}")
                continue
            print(f"[STM32 RX] command_id={command_id} status={STATUS_NAMES[status]}")
            if status == STATUS_DONE:
                return CommandResult(True, command_id, slot, "DONE", attempt)
