# %% [markdown]
# # 3×3 적재함 EMPTY/OCCUPIED 분류 v2 — MobileNetV3-Small
#
# 이 노트북은 교정된 `rack_roi_dataset_v2`의 고정된 9개 ROI를 `EMPTY` 또는 `OCCUPIED`로 분류하는 기준 모델을 학습한다.
#
# 실행 전:
#
...
NOTEBOOK_SOURCE_PATH = (
    NOTEBOOK_DIR / "02_train_mobilenetv3_small_colab_v2.py"
)
NOTEBOOK_IPYNB_PATH = (
    NOTEBOOK_DIR / "02_train_mobilenetv3_small_colab_v2.ipynb"
)
NOTEBOOK_RELEASE_MANIFEST_PATH = (
    NOTEBOOK_DIR / "02_train_mobilenetv3_small_colab_v2.release.json"
)
...
audit_command = [
    sys.executable,
    "-u",
    str(EXTRACT_ROOT / "tools" / "audit_prepared_dataset_v2.py"),
    "--dataset-root",
    str(EXTRACT_ROOT),
    "--prepared-root",
    str(EXTRACT_ROOT / "prepared" / "rack_roi_dataset_v2"),
    "--config",
    str(EXTRACT_ROOT / "config" / "dataset_v2.json"),
]
subprocess.run(audit_command, check=True)
print("PASS: v2 dataset archive and prepared dataset verified")
...
INPUT_SHA256_PATH = DATA_ROOT / "manifests" / "input_sha256.csv"
DATASET_SUMMARY_PATH = DATA_ROOT / "dataset_summary.json"
DATASET_CONFIG_PATH = EXTRACT_ROOT / "config" / "dataset_v2.json"
ROI_CONFIG_PATH = EXTRACT_ROOT / "config" / "roi_calv2.json"
LABEL_CORRECTIONS_PATH = (
    EXTRACT_ROOT / "manifests" / "label_corrections_v2.csv"
)
SPLIT_CONSUMPTION_PATH = (
    EXTRACT_ROOT / "manifests" / "split_consumption_v2.csv"
)
...
if release_manifest.get("release_name") != EXPECTED_RELEASE_NAME:
    raise RuntimeError("Notebook release name mismatch")
if release_manifest.get("dataset_zip_sha256") != EXPECTED_ZIP_SHA256:
    raise RuntimeError("Notebook release dataset ZIP SHA-256 mismatch")
if int(release_manifest.get("dataset_zip_bytes", -1)) != EXPECTED_ZIP_BYTES:
    raise RuntimeError("Notebook release dataset ZIP size mismatch")
...
if (len(train_dataset), len(validation_dataset)) != (2250, 90):
...
train_class_counts = dataset_class_counts(train_dataset)
validation_class_counts = dataset_class_counts(validation_dataset)
if train_class_counts != {"EMPTY": 1090, "OCCUPIED": 1160}:
    raise RuntimeError(f"Unexpected train class counts: {train_class_counts}")
if validation_class_counts != {"EMPTY": 45, "OCCUPIED": 45}:
    raise RuntimeError(
        f"Unexpected validation class counts: {validation_class_counts}"
    )

print("class_to_idx:", train_dataset.class_to_idx)
print("train:", len(train_dataset), train_class_counts)
print(
    "validation:",
...
if frame_manifest["capture_id"].duplicated().any():
    raise RuntimeError("Duplicate capture_id in frame manifest")
expected_capture_ids = {
    "train": {f"a{number:03d}" for number in range(1, 51)},
    "validation": {f"a{number:03d}" for number in range(71, 81)},
    "test": {f"a{number:03d}" for number in range(81, 91)},
}
observed_capture_ids = {
    split: set(
        frame_manifest.loc[
            frame_manifest["final_split"] == split,
            "capture_id",
        ].tolist()
    )
    for split in expected_capture_ids
}
if observed_capture_ids != expected_capture_ids:
    raise RuntimeError(
        "Exact frame split capture IDs mismatch: "
        f"observed={observed_capture_ids}"
    )
...
if dataset_summary.get("dataset_id") != DATASET_ID:
    raise RuntimeError("Dataset summary ID mismatch")
if dataset_config.get("dataset_id") != DATASET_ID:
    raise RuntimeError("Dataset config ID mismatch")
...
manifest_hashes = {
    ...
    "dataset_summary_sha256": sha256_file(DATASET_SUMMARY_PATH),
    "input_sha256_manifest_sha256": sha256_file(INPUT_SHA256_PATH),
    "dataset_config_sha256": sha256_file(DATASET_CONFIG_PATH),
    "label_corrections_sha256": sha256_file(LABEL_CORRECTIONS_PATH),
    "split_consumption_sha256": sha256_file(SPLIT_CONSUMPTION_PATH),
}
print("Manifest hashes:", manifest_hashes)
print("PASS: exact capture IDs train=a001-a050 validation=a071-a080 test=a081-a090")
...
test_class_counts = dataset_class_counts(test_dataset)
if test_class_counts != {"EMPTY": 45, "OCCUPIED": 45}:
    raise RuntimeError(f"Unexpected test class counts: {test_class_counts}")
print("test:", len(test_dataset), test_class_counts)
...
print(
    "주의: 이 test는 s05에서 L00으로만 촬영한 fresh holdout이며 "
    "현장 성능을 증명하지 않습니다."
)
